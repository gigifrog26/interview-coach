"""Qwen Provider implementation for the Interview Coach System."""
...
class PromptInjectionDetector:
    """Detects and prevents prompt injection attacks in Qwen provider."""
...
class SecurePromptBuilder:
    """Builds secure prompts with proper boundaries for Qwen."""
...
class SecurityMonitor:
    """Monitors for security threats in Qwen provider."""
...
class QwenProvider(LLMProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.base_url = config.get("base_url", "https://api.Qwen.com/v1")
        self.model = config.get("model", "Qwen-chat")
        ...
        self.logger = get_logger("Qwen_provider")
        self._session: Optional[requests.Session] = None

    def initialize(self) -> None:
        """Initialize the Qwen provider."""
        if not self.api_key:
            raise AuthenticationError("Qwen API key is required")
        ...
        self.logger.info(f"Qwen provider initialized with model: {self.model}")
    ...
    def _test_connection(self) -> None:
        """Test the connection to Ali API."""
        try:
            ...
            self.logger.debug("Qwen connection test successful")
        except Exception as e:
            self.logger.error(f"Qwen connection test failed: {str(e)}")
            # Clean up session on connection test failure
            if self._session:
                self._session.close()
                self._session = None
            raise LLMProviderError(f"Failed to connect to Qwen API: {str(e)}")
    ...
    def _make_request(self, request: QwenRequest) -> QwenResponse:
        """Make a request to Qwen API."""
        ...
        if response.status_code == 429:
            ...
            raise AuthenticationError("Invalid Qwen API key")
        elif response.status_code == 400:
            error_data = response.json()
            raise LLMProviderError(f"Qwen API error: {error_data}")
        elif response.status_code != 200:
            raise LLMProviderError(f"Qwen API returned status {response.status_code}")
        
        response_data = response.json()
        # self.logger.info(f"Qwen API response: {response_data}")
        try:
            return QwenResponse(**response_data)
        except Exception as e:
            self.logger.warning(f"Failed to parse Qwen response with strict model: {e}")
    ...
    def _make_request_with_retries(self, request: QwenRequest) -> QwenResponse:
        ...
        for attempt in range(self.retries + 1):
            try:
                ...
                last_exception = e
                self.logger.error(f"Qwen request failed. {str(e)}")
                if attempt < self.retries:
                    wait_time = (2 ** attempt) + (time.time() % 1)
                    self.logger.warning(f"Qwen request failed (attempt {attempt + 1}/{self.retries + 1}), retrying in {wait_time:.2f}s: {str(e)}")
                    time.sleep(wait_time)
                else:
                    self.logger.error(f"Qwen request failed after {self.retries + 1} attempts: {str(e)}")
    ...
    def _handle_error(self, error: Exception) -> None:
        ...
        self.logger.error(f"Qwen provider error: {str(error)}")
